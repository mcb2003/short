...
async fn new_link(mut req: Request<()>) -> tide::Result {
    let new_link: NewLink = req
        .body_json()
        .await
        .map_err(|_| Error::from_str(StatusCode::BadRequest, "Invalid JSON payload"))?;
    ...
    Ok(if let Some(link) = Link::by_id(id).await? {
        if !link.deleted() {
            ...
            Response::builder(StatusCode::Ok)
                .header("Last-Modified", link.updated_at().to_rfc2822())
                .body(body)
                .build()
        } else {
...
async fn update_link(mut req: Request<()>) -> tide::Result {
    ...
    let last_modified = if let Some(date) = req.header("If-Unmodified-Since") {
        let date = date.last();
        chrono::DateTime::parse_from_rfc2822(date.as_str())
            .map_err(|_| {
                Error::from_str(
                    StatusCode::BadRequest,
                    "Invalid date in If-Unmodified-Since header",
                )
            })?
            .naive_utc()
    } else {
        return Err(Error::from_str(
            ...
            "Must provide an If-Unmodified-Since header",
        ));
    };
    ...
    Ok(
        if let Some(new) = update.update_if(id, last_modified).await? {
            // Link hasn't been modified since last_modified
            let body = Body::from_json(&new)?;
            Response::builder(StatusCode::Ok)
                .body(body)
                .header("Last-Modified", new.updated_at().to_rfc2822())
                .build()
        } else {
